...
$con=mysqli_connect(HOST,USER,PASS,DB); 
  ...
  $sel=mysqli_query($con,"SELECT p.case_id, of.name as 'facility_of_origin', f.name AS source, r.name AS destination, pm.taken_at, pm.delivered_at, pm.recieved_at, pm.longitude, pm.latitude FROM `packagemovement` pm 
  INNER JOIN facility f ON (f.id = pm.source) 
  INNER JOIN facility r ON(r.id = pm.destination)
  INNER JOIN package p ON(pm.packageid= p.id) 
  LEFT JOIN facility of ON (of.id = p.facilityid) 
  WHERE pm.destination != p.final_destination ORDER BY p.id")or die(mysqli_error($con));
...
function addPackageEvent()
{
	...
	if(isset($_REQUEST['package_id'])){
	 ...
	 mysqli_query($con,"INSERT INTO packagemovement_events set package_id='$package_id',source='$source' ,destination='$destination',status='$status',created_by='$created_by'")or die(mysqli_error($con)); 
	 $id = mysqli_insert_id($con); // last inserted id 
