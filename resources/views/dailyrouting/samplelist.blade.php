...
<div class="well firstrow list">
  <div class="row">
    {{ Form::open(array('route' => 'dailyrouting.samplelist', 'class' => 'form-search pull-left', 'id' => 'samplelist')) }}
            {{ csrf_field() }}
  ...
  <div class="box-body table-responsive">
    <table id="listtable" class="table table-striped table-bordered">
      ...
      <tr>
        ...
        <td>{{$sample->SickleCell}}</td>
        <td>{{$sample->CD4CD8}}</td>        
        <td>{{$sample->CBCFBC}}</td>
        <td>{{$sample->LFTS}}</td>
        <td>{{$sample->RFTS}}</td>
        <td>{{$sample->Culturesensitivity}}</td>
        <td>{{$sample->MTBCultureDST}}</td>
      </tr>
      @endforeach
        </tbody>
